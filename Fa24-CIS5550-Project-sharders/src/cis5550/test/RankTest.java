...
public class RankTest extends GenericTest {
	...
	void installCrawl(Map<String, String> data) throws IOException {
		...
		void installIndex(Map<String, String> data) throws IOException {
		KVSClient kvs = new KVSClient("localhost:8000");
		HTTP.Response response = HTTP.doRequest("GET", "http://" + kvs.getWorkerAddress(0) + "/tables", null);
		if (response != null) {
		    String[] listOfTables = new String(response.body()).split("\n");
		...
		for (String s : data.keySet()) {
			kvs.put("pt-index", s, "acc", data.get(s));
		}
	}
...
//			data.put("apples", "apples bananas coconuts");
//			data.put("bananas", "apples bananas coconuts");
//			data.put("coconuts", "apples bananas coconuts");
//			data.put("http://foo.com/page1.html", "apples bananas coconuts");
