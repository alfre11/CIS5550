...
public class SmallRankerTest extends GenericTest {
...
//		}
	...
	void runTests(Set<String> tests) throws Exception {
		...
		if (tests.contains("indexer")) try{
			...
			data.put("http://foo.com/page1.html", "who which apples bananas coconuts dates");
			data.put("http://foo.com/page2.html", "dates elderberries are figs hazelnuts");
			data.put("http://foo.com/page3.html", "guavas the hazelnuts apples");
			installCrawl(data);
			...
			expected.put("apples", "http://foo.com/page1.html,http://foo.com/page3.html");
			expected.put("bananas", "http://foo.com/page1.html");
			expected.put("coconuts", "http://foo.com/page1.html");
			expected.put("dates", "http://foo.com/page1.html,http://foo.com/page2.html");
			...
			expected.put("hazelnuts", "http://foo.com/page2.html,http://foo.com/page3.html");
...
//			String problems = "";
		...
		if (tests.contains("ranker")) try{
			startTest("ranker", "Ranker: Simple tests", 40);
			...
			try {
				...
				String output = FlameSubmit.submit("localhost:9000", "ranker.jar", "cis5550.jobs.Ranker",
						new String[] { "apples bananas the figs" });
				System.out.println("returned");
				if (output == null)
					testFailed("Looks like we weren't able to submit 'ranker.jar'; the response code was "
							+ FlameSubmit.getResponseCode() + ", and the output was:\n\n"
							+ FlameSubmit.getErrorResponse());
				System.out.println("passed? "+output);
			} catch (FileNotFoundException fnfe) {
				testFailed("Looks like 'ranker.jar' was not found in the current directory.");
			}
	...
	public static void main(String args[]) throws Exception {
		...
		SmallRankerTest t = new SmallRankerTest();
