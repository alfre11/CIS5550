...
class Coordinator extends cis5550.generic.Coordinator {
  ...
  public static KVSClient kvs;
    ...
    post("/submit", (request,response) -> {
      ...
      Thread threads[] = new Thread[getWorkersList().size()];
      String results[] = new String[getWorkersList().size()];
      for (int i=0; i<getWorkersList().size(); i++) {
        final String url = "http://"+getWorkersList().elementAt(i)+"/useJAR?kvs=" + args[1];
        final int j = i;
      ...
      try {
    	FlameContextImpl fci = new FlameContextImpl(jarName);
        Loader.invokeRunMethod(jarFile, className, fci, argVector);
        return fci.getOutput();
