...
public class Indexer {
	...
	public static void run(FlameContext ctx, String[] args) {
		try {
			...
			FlamePairRDD wordToUrls = urlPagePairs.flatMapToPair(row -> {
				String url = row._1();
...
//				}
				...
				for (int i = 0; i < words.length; i++) {
					if (!words[i].isEmpty() && !stopWords.containsKey(words[i])) {
				...
				wordPositions.forEach((word, positions) -> {
					result.add(new FlamePair(word, url));
				});
			...
			FlamePairRDD wordToTf = urlPagePairs.flatMapToPair(row -> {
				...
				wordCount.forEach((word, count) -> {
					double tf = (double) count / totalWords; // Calculate term frequency
					result.add(new FlamePair(word, url + "," + tf)); // Save TF for pt-tfidf
				});
			...
			FlamePairRDD indexedWords = wordToUrls.foldByKey("",
					(a, b) -> a + (a.equals("") ? "" : ",") + b
			);

			indexedWords.saveAsTable("pt-index");

			FlamePairRDD indexedWordsTF = wordToTf.foldByKey("",
					(a, b) -> a + (a.equals("") ? "" : ",") + b
			);
//			indexedWordsTF.saveAsTable("pt-tfidf");
			...
			FlamePairRDD tfIdf = tfDfJoined.flatMapToPair(pair -> {
				...
				List<FlamePair> result = new ArrayList<>();
				result.add(new FlamePair(word, url + "," + tfIdfScore));
				return result;
			});

			// Save the TF-IDF results
			//kinda restarted for now
